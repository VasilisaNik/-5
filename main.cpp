...
int main()
{
    ...
    wcout << endl << L"Играем во все бесплатные игры с помощью итератора:" << endl;
    Iterator<Game*> *freeIt = new GameFreeDecorator(gameArray.GetIterator(), true);
    PlayAll(freeIt);
    delete freeIt;

    wcout << endl << L"Играем во все игры с рейтингом 52:" << endl;
    ...
    wcout << endl << L"Играем во все очень большие игры:" << endl;
    Iterator<Game*> *ochenIt = new GameRazmerDecorator(gameArray.GetIterator(), GameRazmer::Ochen_mnogo);
    PlayAll(ochenIt);
    delete ochenIt;

    // Демонстрация работы адаптера
    wcout << endl << L"Играем во все бесплатные игры с рейтингом 100 с помощью adapted iterator (другой контейнер):" << endl;
